...
from ibmsdnve.common import config
from ibmsdnve.common import constants
from ibmsdnve.common import exceptions as sdnve_exc
from ibmsdnve.ml2 import sdnve_api as sdnve
from ibmsdnve.ml2 import sdnve_api_fake as sdnve_fake
...
class SdnveDriver(object):
    ...
    def _create_network(self, context):
        ...
        LOG.debug(_("Create network in progress: %r"), processed_request)
        (res, data) = self.sdnve_client.sdnve_create('network', processed_request)
    ...
    def _update_network(self, context):
        ...
        if processed_request['network']:
            (res, data) = self.sdnve_client.sdnve_update(
                              'network', id, processed_request)
            if res not in constants.HTTP_ACCEPTABLE:
                raise sdnve_exc.SdnveException(
                        msg=(_('Update net failed in SDN-VE: %s') % data))
    ...
    def _update_port(self, context):
        ...
        processed_request = {}
        need_sg_member_updated = context._plugin.is_security_group_member_updated(
            context._plugin_context, context.original, context.current)
        LOG.debug(_("is sg_member_updated %s"), str(need_sg_member_updated))
        ...
        if need_sg_member_updated:
            LOG.debug(_("calling sg member update on orig port"))
            context._plugin.notify_security_groups_member_updated(context._plugin_context, context.original)
            LOG.debug(_("calling sg member update on new port"))
            context._plugin.notify_security_groups_member_updated(context._plugin_context, context.current)

        processed_request['port'] = self.filter_update_port_attributes(
                                         self.sdnve_client._process_update(
                                            context.current, context.original))
    ...
    def _create_subnet(self, context):
        ...
        (res, data) = self.sdnve_client.sdnve_create('subnet', processed_request)
        if res not in constants.HTTP_ACCEPTABLE:
            raise sdnve_exc.SdnveException(
    ...
    def _update_subnet(self, context):
        ...
        if processed_request['subnet']:
            ...
            (res, data) = self.sdnve_client.sdnve_update(
                              'subnet', context.current['id'], processed_request)
            if res not in constants.HTTP_ACCEPTABLE:
                raise sdnve_exc.SdnveException(
    ...
    def check_ip_pool_overlap(self, context, subnet):
        ...
        new_network = context._plugin.get_network(dbcontext, subnet["network_id"])
        new_shared_internal = new_network['shared'] and not new_network['router:external']
        ...
        for pool in allocation_pools:
            allocations = allocations | netaddr.IPSet(netaddr.IPRange(pool['first_ip'], pool['last_ip']))
        ...
        for sub in subnets:
            ...
            if network["id"] == new_network["id"]:
                continue
            old_shared_internal = network['shared'] and not network['router:external']
            ...
            if (new_shared_internal and old_external) or \
                ...
                for pool in pool_qry.filter_by(subnet_id=sub['id']):
                    ...
                    if len(diff):
                        LOG.info(_("external subnet %s pool overlap with %s"),
                                 subnet)
                        return (True, sub["id"])
        return (False, None)                    
    ...
    def _check_subnet_create(self, context, subnet):
        ...
        if ret:
            raise sdnve_exc.SdnveException(
                msg=(_('Create subnet failed in SDN-VE, IP pool overlap with subnet %s') % subid))
        # check cidr overlaps
        ret, subid = self.check_subnet_cidr_overlap(context, subnet)
        if ret:
            raise sdnve_exc.SdnveException(
                msg=(_('Create subnet failed in SDN-VE, CIDR overlap with subnet %s') % subid))
    ...
    def _filter_update_request(self, resource, type):
       ...
       for key, value in resource.items():
           if key in restricted_values:
               msg = _("Update of %s is not supported by SDNVE Controller") % key
    ...
    def _clear_floating_ip(self,context):
        ...
        if l3plugin:
            ...
            floatingip_list = l3plugin.get_floatingips(dbcontext,floatingip_filter)
            for floatingip in floatingip_list:
                LOG.debug(_("Updating floating IP before port delete %r"), floatingip)
                floatingip_id = floatingip.get('id')
                if floatingip_id:
                    #update floating-ip object
                    (res, data) = self.sdnve_client.sdnve_update(
                                  'floatingip', floatingip_id,
                                  {'floatingip': {}})
                    if res not in constants.HTTP_ACCEPTABLE:
                        LOG.error(_("Floating ip disassociate failed in SDN-VE "
                                    ": %s"), data)

    def filter_update_port_attributes(self, port):
        """Filter out port attributes for an update operation."""
        self.try_del(port, ['security_groups', 'status', 'fixed_ips', 'admin_state_up'])
        return port
    ...
    def handleEmptyTenant(self, context, resource, type):
        ...
        if type == 'port':
            ...
            if network_id:
                network_info = context._plugin.get_network(dbcontext, network_id)
